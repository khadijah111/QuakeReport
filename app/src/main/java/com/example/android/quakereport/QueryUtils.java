...
public final class QueryUtils {
    ...
    /**
     * Return a list of {@link EarthQuacks} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<EarthQuacks> extractEarthquakes() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<EarthQuacks> earthquakes = new ArrayList<>();
        ...
        try {
            ...
            JSONArray quackArrays = root.getJSONArray("features");

                      // looping through All Quacks
            for (int i = 0; i <quackArrays.length(); i++) {
                JSONObject currentEarthQuake = quackArrays.getJSONObject(i);// for each quack
                JSONObject properties = currentEarthQuake.getJSONObject("properties");
                Double quakeMagnitude = properties.getDouble("mag");
                String quakeLocation = properties.getString("place");
                Long quakeDate = properties.getLong("time");
                String quakeUrl = properties.getString("url");
                //call class instructor to fill earthQuack object
                EarthQuacks earthQuake = new EarthQuacks(quakeMagnitude, quakeLocation, quakeDate,quakeUrl);
